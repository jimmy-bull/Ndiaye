...
const App = ({ navigation }) => {
    ...
    const viewConfigRef = useRef({ itemVisiblePercentThreshold: 50, waitForInteraction: true, minimumViewTime: 5, })
    const scrollX = [];
    ...
    const renderItem = ({ item }) => {
        return (
            <Item posterName={item.posterName} city={item.city} country={item.country} postType={item.postType}
                images={item.images} id={item.id} description={item.description} equipe1={item.equipe1} equipe2={item.equipe2}
                logo1={item.logo1} logo2={item.logo2} date={item.date} sport={item.sport} score1={item.score1} score2={item.score2} idImageSub={item.idImageSub} />)
    };
    const Item = (({ posterName, city, country, description, images, postType, equipe1, equipe2, logo1, logo2,
        date, sport, score1, score2, id, }) => {
        ...
        const field2 = (item) => {
            if (item.item.image.split('.')[1] != 'mp4' && item.item.id == 0) {
                return (
                    <View style={{
                        ...
                        <View style={{ marginBottom: 0, justifyContent: 'flex-end', flex: 1 }}>
                            {
                                <Image style={{ height: "auto", width: windowWidth, aspectRatio: item.item.imageDimension, backgroundColor: "lightgray", position: "relative", zIndex: 1, resizeMode: 'cover' }} defaultSource={require('../image/wainting.png')} source={{ uri: imageDomaine + item.item.image, cache: 'default' }} />
                            }
                ...
                return (
                    <View style={{ aspectRatio: item.item.imageDimension, width: Dimensions.get('window').width }}>
                        <VideoPlayer
                            source={{ uri: imageDomaine + item.item.image }}
                            style={{ aspectRatio: item.item.imageDimension, width: Dimensions.get('window').width, }}
        ...
        if (postType == 'automatique') {
            return (
                <View>
                    <View style={{
                        ...
                        <Text style={{ alignSelf: "center", fontSize: 18, fontWeight: "500", color: "black" }}>{sport}</Text>
                        <Text style={{ alignSelf: "center", color: "gray", marginTop: 10 }}>{date}</Text>
                        <View style={{ flexDirection: "row", justifyContent: "space-evenly", marginTop: 20, alignItems: "center" }}>
                            <View style={{ alignItems: "center" }}>
                                <Image style={{ height: 70, width: 70, marginBottom: 10, resizeMode: 'contain' }} defaultSource={require('../image/wainting.png')} source={{ uri: logo1, cache: 'default' }} />
                                <Text style={{ fontWeight: "500", color: "black" }}>{equipe1}</Text>
                            </View>
                            <View style={{ flexDirection: "row", }}>
                                <View>
                                    <Text style={{ fontSize: 25, fontWeight: "bold", padding: 5, color: "black", paddingTop: 0 }}>{score1}</Text>
                                </View>
                                <View>
                                    <Text style={{ fontSize: 25, fontWeight: "bold", padding: 5, color: "black", paddingTop: 0 }}>:</Text>
                                </View>
                                <View>
                                    <Text style={{ fontSize: 25, fontWeight: "bold", padding: 5, color: "black", paddingTop: 0 }}>{score2}</Text>
                                </View>
                            </View>
                            <View style={{ alignItems: "center" }}>
                                <Image style={{ height: 70, width: 70, marginBottom: 10, resizeMode: 'contain' }} defaultSource={require('../image/wainting.png')} source={{ uri: logo2, cache: 'default' }} />
                                <Text style={{ fontWeight: "500", color: "black" }}>{equipe2}</Text>
                            </View>
                        </View>
                    </View>
                    <View>
                        <BottomPost description={description} id={id} color='white' imageDataState={imageDataState} navigation={navigation} />
                    </View>
                </View>
            )
            ...
            return (
                <>
                    ...
                    <BottomPost description={description} id={id} color='white' navigation={navigation} imageDataState={imageDataState} />
                </>
            )
        }
    });
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: 'rgba(220,220,220,0.2)' }}>
            <StatusBar
                animated={true}
                backgroundColor="white"
                barStyle={"dark-content"}
            />
            <View style={{ flexDirection: "row", justifyContent: 'space-between', alignItems: "center", paddingTop: 10, padding: 5, paddingBottom: 0, backgroundColor: "white", }}>
                <Image style={{ height: 60, width: 60, alignSelf: "center", }} defaultSource={require('../image/wainting.png')} source={require('../image/logo.png')} />
                <View style={{ alignItems: "center", flexDirection: 'row', justifyContent: "space-evenly" }}>
                    <View>
                        <TouchableOpacity onPress={() => navigation.navigate('TeamPage')} style={{ backgroundColor: "white", width: 35, height: 35, marginLeft: 10, justifyContent: 'center' }}>
                            {/* <Icon style={{ alignSelf: 'center' }} onPress={() => bottomSheetNotification.current.show()} name="bell" color='black' size={20} /> */}
                            <Ionicons style={{ alignSelf: 'center' }} name="people-outline" size={20} color="black" />
                        </TouchableOpacity>
                    </View>
                    <View>
                        <TouchableOpacity style={{ backgroundColor: "white", width: 35, height: 35, marginLeft: 10, justifyContent: 'center' }}>
                            {/* <Icon style={{ alignSelf: 'center' }} onPress={() => bottomSheetNotification.current.show()} name="bell" color='black' size={20} /> */}
                            <Ionicons style={{ alignSelf: 'center' }} name="game-controller-outline" size={20} color="black" />
                        </TouchableOpacity>
                    </View>
                    {/* 
                    <View>
                    ...
                    <TouchableOpacity onPress={() => navigation.navigate('Search')} style={{ backgroundColor: "white", width: 35, height: 35, marginLeft: 10, justifyContent: 'center' }}>
                        <Icon onPress={() => navigation.navigate('Search')} style={{ alignSelf: 'center' }} name="search" size={20} color={'black'} />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => navigation.navigate('Profile')} style={{ backgroundColor: "white", width: 35, height: 35, marginLeft: 10 }}>
                        <Image style={{ height: 35, width: 35, borderRadius: 17.5 }} defaultSource={require('../image/wainting.png')} source={{ uri: "https://www.didierdrogbafoundation.org/sites/default/files/drogba.jpg", cache: 'default' }} />
                    </TouchableOpacity>
                </View>
            </View>
            <ScrollView>
                {
                    imageData.map((data, key) => (
                        <>
                            ...
                            <BottomPost description={data.description} id={data.id} color='white' navigation={navigation} imageDataState={imageDataState} />
                        </>
                    ))
                }
            </ScrollView>
